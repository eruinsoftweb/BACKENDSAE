...
const registrarPago = async (req, res = response) => {
    try {
        ...
        // Reducción de la cantidad de uniformes si se proporcionaron
        if (req.body.uniforme && req.body.uniforme.length > 0) {
            for (const uniformeId of req.body.uniforme) {
                const uniforme = await Uniforme.findById(uniformeId);
                if (uniforme) {
                    uniforme.cantidad -= 1; // Disminuir la cantidad requerida por el pago
                    await uniforme.save(); // Guardar los cambios en la cantidad del uniforme
                }
            }
        }

        // Populate del modelo de pago para obtener la referencia al estudiante
        const pagoPopulado = await Pago.findById(pago._id).populate('estudiante').exec();
